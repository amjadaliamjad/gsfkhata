...
// 1. Replace formatWhoOwesWhom
const oldFormat = `function formatWhoOwesWhom(amount, name) {
...
const newFormat = `function formatWhoOwesWhom(amount, name) {
    if (amount === 0) return 'کوئی بقایا نہیں';
    if (amount > 0) return \`حافظ خادم نے \${name} کو دینے ہیں\`;
    return \`\${name} نے حافظ خادم کو دینے ہیں\`;
}`;

code = code.replace(/function formatWhoOwesWhom\([\s\S]*?\}\n/, newFormat + '\n');

// 2. Rewrite renderCashProfit
const newCashProfit = `export function renderCashProfit(config, calc, idParam) {
    const s2 = calc.s2;
    ...
    let topMenuHTML = \`
        <div style="display:flex; flex-wrap:wrap; gap:10px; margin-bottom:25px; justify-content:center; background:var(--w); padding:15px 20px; border-radius:12px; box-shadow:var(--s1); border:1px solid var(--g200);">
            \${validMembers.map(m => \`
                <a href="#cashprofit/\${m.id}" style="padding:8px 16px; border-radius:20px; text-decoration:none; font-size:15px; transition:all 0.2s; \${m.id === idParam ? 'background:var(--gd); color:var(--w); font-weight:bold; box-shadow:0 4px 10px rgba(27,67,50,0.3); transform:translateY(-2px);' : 'background:var(--g100); color:var(--g800); border:1px solid var(--g300);'}">
                    \${m.name}
                </a>
            \`).join('')}
        </div>
    ...
    if (activeMember) {
        ...
        let txRows = m.cashTransactions.map(tx => {
            ...
            let txDetail = tx.baseAmount > 0 ? 'حافظ خادم نے دینے ہیں' : 'بھائی نے خادم کو دینے ہیں';
        ...
        <div class="table-container" style="box-shadow:var(--s2); border:1px solid \${headerColor}; margin-bottom:30px;">
            ...
            <div style="background:var(--w); padding:10px 20px; font-size:14px; color:var(--g600); border-bottom:1px solid var(--g200);">
                <strong>نوٹ:</strong> یہ ٹیبل انفرادی لین دین دکھاتا ہے۔
            </div>
            <div class="tw">
...
if (startIdx !== -1 && endIdx !== -1) {
    code = code.substring(0, startIdx) + newCashProfit + '\n\n' + code.substring(endIdx);
    fs.writeFileSync(file, code);
    console.log("Successfully updated renderCashProfit and formatWhoOwesWhom.");
} else {
