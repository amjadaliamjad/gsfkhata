...
const regexHeaders = /<th style="background:var\(--w\);color:var\(--g800\);border-bottom:2px solid var\(--gm\)">بنیادی حصہ<br>\(بغیر منافع\)<\/th>[\s\S]*?<th style="background:var\(--w\);color:var\(--g800\);border-bottom:2px solid var\(--gm\)">کل کرایہ حصہ<br>\(منافع کیساتھ\)<\/th>\s*<th style="background:var\(--w\);color:var\(--g800\);border-bottom:2px solid var\(--gm\)">پہلے سے وصول شدہ<\/th>\s*<th style="background:var\(--w\);color:var\(--g800\);border-bottom:2px solid var\(--gm\)">نیٹ بقایا وصولی<\/th>/;

const newHeaders = `<th style="background:var(--w);color:var(--g800);border-bottom:2px solid var(--gm)">حتمی فکسڈ بقایا<br>(D - C)</th>
...
if (!regexHeaders.test(code)) {
    console.log("Could not find headers regex!");
    process.exit(1);
}

code = code.replace(regexHeaders, newHeaders);

// Now update the row loop logic
const oldRowLogic = `        let received = m.receivedRent || 0;
...
const newRowLogic = `        let profit = rentWithProfit - baseRent;
        let profitRate = baseRent > 0 ? ((profit / baseRent) * 100).toFixed(1) : 0;
...
// Now update the HTML for the row inside memberRows
const oldRowHtml = `<td class="n">\${num(baseRent)}</td>
...
const newRowHtml = `<td class="n">\${m.isAdmin ? '—' : num(finalPayableBase)}</td>
            <td style="color:var(--gm);font-weight:bold;background:var(--gb);">\${profitCellHtml}</td>
            <td class="n" style="color:var(--blu);font-weight:bold;">\${profitRate}%</td>
...
console.log('Updated ui.js table logic!');
