...
if(uiJs.includes(oldTile3)) {
    uiJs = uiJs.replace(oldTile3, newTile3);
}

// 2. Wrap Option 1
let option1Start = `    <!-- OPTION 1 SECTION -->`;
let option1End = `    <br><hr style="border:none;border-top:2px dashed var(--g300);margin:30px 0;"><br>`;

if (uiJs.includes(option1Start)) {
    uiJs = uiJs.replace(option1Start, `\${(config.agreementMode === 1) ? \`\n    <!-- OPTION 1 SECTION -->`);
}

if (uiJs.includes(option1End)) {
    uiJs = uiJs.replace(option1End, `\` : ''}\n\n    \${(config.agreementMode === 2 || !config.agreementMode) ? \`\n    <br><hr style="border:none;border-top:2px dashed var(--g300);margin:30px 0;"><br>`);
}

// 3. Wrap Option 2 (close at the end of renderDashboard)
// We look for the exact closing of renderDashboard:
let dashboardEnd = `        <div class="table-expl">
...
if (uiJs.includes(dashboardEnd)) {
    uiJs = uiJs.replace(dashboardEnd, newDashboardEnd);
}

// 4. Update Cash Profit logic
...
console.log('ui.js beautifully rewritten!');
